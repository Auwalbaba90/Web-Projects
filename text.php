...
<body>
	<div id="container">
		...
		<div class="row">
			...
			<form action="text.php" id="candidate" method="Post" name="candidate">
			<div class="full-col">
				<div class="full-col">
					<?php
						if (isset($_POST['Add_candidate'])){
							if(isset($_POST['Name'], $_POST['School'], $_POST['Course'], $_POST['Local_govt'], $_POST['Position'], $_POST['image']));{
								...
								$image_name_ext = substr($image, strpos($image, '.') + 1);
								$image_new_name = date('ynjgi') .$image_name_ext;
								$image_path = './candidate_image/'.$image_new_name;
										...
										if(!empty($Course)){
											...
											if(!empty($Local_govt)){
												...
												if(!empty($Position)){
													...
													if($image_name_ext == 'jpg' || $image_name_ext == 'jpeg' || $image_name_ext == 'png'){
														if($image_type == 'image/jpg' || $image_name_ext == 'image/jpeg' || $image_name_ext == 'image/png'){
															$location = './candidate_image/'.$image_new_name;
																if(move_uploaded_file($image_tmp_name, $location)){
																require_once('../db_connection/db_connect.php');{
													$query="INSERT INTO candidate (Full_Name, School, Course, Local_Govt, Position_Name, Candidate_Photo) VALUES (?,?,?,?,?,?)";
													$stmt=mysqli_prepare($dbc,$query);
													mysqli_stmt_bind_param($stmt,"ssssss",$Name,$School,$Course,$Local_govt,$Position,$location);
													...
													echo '<b style="color: red;"> Error: Please Try Again.</b>';
												}
											}else{
												echo '<b style="color: red;"> Please Choose a Photo.</b>';
											}
										}else{
											echo '<b style="color: red;">File Must Be an Image!</b>';
										}
									}else{
	...
	?>
				...
				<div class="two"><br>
					<label class="leb"><b>Upload Candidate Photo:</b><span style="color: red;">*</span></label><br>
					<input class="photo" type="file" name="image" title="Upload Image only" ><br>
				</div>

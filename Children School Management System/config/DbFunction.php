...
class DbFunction{
	...
	$db = Database::getInstance();
	$mysqli = $db->getConnection();
	$query = "SELECT Staff_ID, Password FROM staff_account where Staff_ID=? and Password=? ";
	...
	function valid($Addmission_No,$Class){
			
	$db = Database::getInstance();
	$mysqli = $db->getConnection();
	$query = "SELECT Addmission_No, Class FROM students where Addmission_No=? and Class=? ";
	$stmt= $mysqli->prepare($query);
	if(false===$stmt){
		...
		$stmt->bind_param('ss',$Addmission_No,$Class);
		$stmt->execute();
		$stmt -> bind_result($Addmission_No,$Class);
		$rs=$stmt->fetch();
		if(!$rs)
		{
			echo "<script>alert('Invalid Student Credential, Confirm Student ID Or Class and Try Again!!.')</script>";
			//header('location:login.php');
		}
		else{
	...
	function valid2($Addmission_No,$Class){
	...
	function check($Reg_No,$Term,$c_Session){
			
	$db = Database::getInstance();
	$mysqli = $db->getConnection();
	$query = "SELECT Addmission_No, Term, Session FROM primary_result where Addmission_No=? and Term=? and Session=? ";
	$stmt= $mysqli->prepare($query);
	if(false===$stmt){
		...
		$stmt->bind_param('sss',$Reg_No,$Term,$c_Session);
		$stmt->execute();
		$stmt -> bind_result($Reg_No,$Term,$c_Session);
		$rs=$stmt->fetch();
		if(!$rs)
		{
			echo "<script>alert('Invalid Student Credential, Confirm Student ID Or Class and Try Again!!.')</script>";
